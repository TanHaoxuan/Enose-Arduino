...
for oversample in (False, True ):  
    ...
    if balancing_data:
        ...
        if oversample:
            # For oversampling, find the maximum count to match all groups to
            max_count = counts['counts'].max()
            
            def resample_group(group):
                return group.sample(max_count, replace=True, random_state=42)
        else:
            # For undersampling, continue to use the minimum count as before
            min_count = counts['counts'].min()
            
            def resample_group(group):
                return group.sample(min_count, replace=False, random_state=42)
